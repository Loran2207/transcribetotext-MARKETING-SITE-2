...
export function Guarantee() {
  return (
    <div className="mt-10 rounded-tile border border-accent/15 bg-accent-soft/50 px-5 py-7 text-center sm:px-8 sm:py-8 lg:mt-12">
      ...
      <p className="mt-4 font-display text-xl font-bold tracking-tight text-ink sm:text-2xl">{subscribe.guarantee.title}</p>
...
// Bottom block of the paywall, centered: the shared secure-checkout card, help
// email, publisher details and legal links.
export function SafeCheckout() {
  return (
    <div className="mt-12 flex flex-col items-center border-t border-border pt-10 text-center lg:mt-16 lg:pt-12">
      <SecureCheckout className="max-w-md" />
      <p className="mt-8 font-display text-lg font-bold text-ink">{subscribe.help.title}</p>
      ...
      <p className="mt-8 font-display text-lg font-bold text-ink">{subscribe.publisher.title}</p>
