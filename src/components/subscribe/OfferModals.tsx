...
// `fullscreenOnMobile` is opt-in: below sm the dialog becomes the whole viewport
// (no backdrop, no corners, no margin) and reads as a page. Every other offer
// modal keeps the centred card on a blurred dark backdrop.
function OfferShell({ open, onClose, maxW, children, id, fullscreenOnMobile = false }: { open: boolean; onClose: () => void; maxW: string; children: ReactNode; id: string; fullscreenOnMobile?: boolean }) {
  return (
    <AnimatePresence>
      {open && (
        <motion.div className={`fixed inset-0 z-[70] flex items-center justify-center ${fullscreenOnMobile ? "p-0 sm:p-4" : "p-4"}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          <div className={`absolute inset-0 ${fullscreenOnMobile ? "bg-white sm:bg-ink/60 sm:backdrop-blur-sm" : "bg-ink/60 backdrop-blur-sm"}`} onClick={onClose} />
          <motion.div
            id={id}
            ...
            className={`relative w-full max-w-none overflow-y-auto bg-white ${fullscreenOnMobile ? "h-dvh max-h-dvh rounded-none px-5 pb-12 pt-6 sm:h-auto sm:max-h-[92dvh] sm:rounded-tile sm:p-8 sm:shadow-lift" : "max-h-[92dvh] rounded-tile p-6 shadow-lift sm:p-8"} ${maxW}`}
          >
...
export function OfferCheckoutModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <OfferShell open={open} onClose={onClose} maxW="sm:max-w-5xl" id="offer-checkout" fullscreenOnMobile>
      <div className="flex flex-col items-center gap-3">
        <img src="/brand/logo.svg" alt="TranscribeToText.AI" className="h-7 w-auto" />
        <h3 className="text-center font-display text-2xl font-extrabold tracking-tight text-ink sm:text-3xl">{o.title}</h3>
      </div>
      <div className="mt-7 grid gap-8 lg:grid-cols-[1fr_420px] lg:gap-9">
        <div className="order-2 border-t border-border-soft pt-8 sm:border-0 sm:pt-0 lg:order-1">
        ...
        <div className="order-1 bg-white sm:rounded-tile sm:border sm:border-border sm:p-6 lg:order-2">
          ...
          <div className="flex items-baseline justify-between gap-3">
            <p className="whitespace-nowrap font-display text-base font-bold text-ink">{o.totalLabel}</p>
          ...
          <p className="flex items-center gap-2 text-sm text-ink-2">
            ...
            <span>
              {o.guarantee.pre} <span className="font-semibold text-ink">{o.guarantee.strong}</span> {o.guarantee.post}
            </span>
          </p>
          ...
          <div className="my-5 flex items-center gap-3">
            ...
            <span className="whitespace-nowrap text-xs font-medium text-muted">{o.cardDivider}</span>
            <span className="h-px flex-1 bg-border-soft" />
          </div>

          <div className="space-y-2.5">
            <input value={card} onChange={(e) => setCard(e.target.value)} inputMode="numeric" placeholder={o.cardPlaceholder} className={field} />
            <div className="grid grid-cols-2 gap-2.5">
              ...
              <div className="relative">
                ...
                <Info size={16} className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-muted" />
              </div>
            </div>
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder={o.namePlaceholder} className={field} />
          </div>

          <button onClick={onClose} className={`mt-5 ${GRADIENT_PILL}`}>{o.cta} <ArrowRight size={16} /></button>
          <div className="mt-5"><SecureCheckout /></div>
          <p className="mt-3 text-center text-[11px] leading-relaxed text-muted">
            {o.termsPre}{" "}
            {o.termsLinks.map((l, i) => (
              <span key={l}>
                <a href="#" className="font-medium text-ink-2 underline underline-offset-2 transition-colors hover:text-accent">{l}</a>
