    ...
    <section className="border-y border-border bg-white py-14 md:py-16">
      <Container>
        ...
        <motion.dl
          ...
          whileInView="show"
          viewport={viewportOnce}
          className="mx-auto mt-10 grid max-w-5xl grid-cols-2 gap-y-10 md:mt-12 lg:grid-cols-4"
        >
          {trust.cards.map((c, i) => (
            <motion.div
              key={c.label}
              variants={scaleIn}
              className={`flex flex-col items-center gap-1.5 px-4 text-center ${i > 0 ? "lg:border-l lg:border-border-soft" : ""}`}
            >
              {i === 3 ? (
                <dd className="flex h-[44px] items-center gap-3 md:h-[48px]">
                  {MEETING_MARKS.map((name) => (
                    <img key={name} src={platform[name]} alt={name} className="h-7 w-auto md:h-8" />
                  ))}
                </dd>
              ) : (
                <dd>
