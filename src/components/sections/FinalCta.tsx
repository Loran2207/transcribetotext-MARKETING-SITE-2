  ...
  return (
    <section className="relative overflow-hidden bg-dark-atmosphere py-20">
      ...
      <Container className="relative">
        <motion.h2 variants={blurIn} initial="hidden" whileInView="show" viewport={viewportOnce} className="text-balance text-center font-display text-4xl font-extrabold tracking-tight text-white">
          {finalCta.title}
        </motion.h2>
        <motion.div variants={stagger(0.09)} initial="hidden" whileInView="show" viewport={viewportOnce} className="mx-auto mt-14 grid max-w-6xl grid-cols-4 gap-10">
          {finalCta.items.map((it, i) => {
            const Icon = ICONS[i];
            return (
              <motion.div key={it.title} variants={fadeUp}>
                <Icon strokeWidth={1.5} className="mx-auto size-12 text-accent-glow" />
